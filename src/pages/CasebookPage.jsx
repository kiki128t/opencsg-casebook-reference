...
import { capabilities, casebookContent, caseStudies, LEAD_KEY, LOGIN_KEY, relatedResources } from '../data/content'
...
export default function CasebookPage() {
  ...
  return <PageShell><CasebookHero onRead={read} onDownload={download}/><section className="section overview"><div className="container"><div className="section-head centered"><h2>{casebookContent.overview.title}</h2></div><div className="stats-grid">{casebookContent.overview.stats.map(stat => <article className={`stat-card ${stat.featured ? 'featured' : ''}`} key={stat.label}><strong>{stat.value}</strong><span>{stat.label}</span></article>)}</div></div></section><section className="section capabilities"><div className="container"><div className="section-head"><h2>{casebookContent.capabilitySection.title}</h2><p>{casebookContent.capabilitySection.description}</p></div><div className="cap-grid">{capabilities.map((capability,i) => { const Icon=capIcons[i]; return <article key={capability.title}><div className="cap-icon"><Icon/></div><span>0{i+1}</span><h3>{capability.title}</h3><p>{capability.description}</p><ArrowRight/></article>})}</div></div></section><PreviewGallery/><section className="full-read-prompt"><div><p>{casebookContent.readerPrompt.text}</p><button onClick={read}>{casebookContent.readerPrompt.action} <ArrowRight size={17}/></button></div></section><section className="section featured"><div className="container"><div className="section-head"><h2>精选客户实践</h2></div><div className="case-grid four">{caseStudies.slice(0,4).map((x,i)=><CaseCard item={x} key={i}/>)}</div></div></section><section className="download-cta"><div className="container"><div><h2>获取《OpenCSG 客户案例总册》</h2><p>了解 OpenCSG 在不同产业场景中的 AI 落地实践。</p></div><div><SecondaryButton onClick={read}>在线阅读</SecondaryButton><PrimaryButton onClick={download} icon="download">立即下载</PrimaryButton></div></div></section><section className="section related"><div className="container"><div className="section-head"><h2>继续探索</h2></div><div className="resource-grid">{relatedResources.map(x => <RelatedResourceCard item={x} key={x.url}/>)}</div></div></section></PageShell>
}
